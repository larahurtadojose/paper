...
La red de pago Visa logró 47,000 transacciones por segundo (tps) en el punto de carga maxima
durante las vacaciones \cite{visa} de 2013, y actualmente promedia cientos
de millones por día. Actualmente, Bitcoin admite menos de 7 transacciones por
segundo con un límite por bloque de 1 megabyte. Si utilizamos un promedio de 300 bytes por
transacción bitcoin y asumimos tamaños ilimitados de bloques y una capacidad equivalente a
el volumen máximo de transacciones de Visa de 47,000 / tps sería necesario una capacidad 
de casi 8 gigabytes por bloque de Bitcoin, cada diez minutos en promedio. 
Continuamente, eso sería más de 400 terabytes de datos por año.

Claramente, no es factible lograr una capacidad parecida a Visa en la red de Bitcoin
actualmente. Ninguna computadora hogareña en el mundo puede operar con ese tipo de ancho de banda y
almacenamiento. Si Bitcoin reemplazará todos los pagos electrónicos en el futuro, y no
solo Visa, resultaría en un colapso absoluto de la red de Bitcoin, o en
en el mejor caso, la centralización extrema de los nodos y mineros de Bitcoin a los únicos que
puedan pagarlo. Esta centralización vencería aspectos de la descentralización de la red
que hacen que Bitcoin sea seguro, ya que la capacidad de las entidades 
para validar la cadena es lo que permite a Bitcoin garantizar la precisión y la seguridad del libro mayor.

Tener menos validadores debido a bloques más grandes no solo implica menos individuos
asegurando la precisión del libro mayor, también resulta en menos entidades que podrían
para validar blockchain como parte del proceso de minería, lo que resulta en
alentar la centralización de los mineros. Bloques extremadamente grandes, por ejemplo en el
caso anterior de 8 gigabytes cada 10 minutos en promedio, implicaría que solo
pocas partes podrían hacer la validación de bloque. Esto crea una gran
posibilidad de que las entidades terminen confiando en las partes centralizadas. Teniendo
las partes privilegiadas y de confianza crean una trampa social por la cual la parte central
no actuar en interés de un individuo (problema principal-agente), por ejemplo
rentismo al cobrar tarifas más altas para mitigar el incentivo de actuar deshonestamente.
En casos extremos, esto se manifiesta como personas que envían fondos a custodios centralizados
de confianza que tienen la custodia total de los fondos de los clientes. Tales arreglos,
como es común hoy en día, crean un riesgo severo de contraparte. Un prerrequisito para prevenir
ese tipo de centralización de ocurrir requeriría la capacidad de bitcoin
para ser validado por una sola computadora de nivel de consumidor en una conexión de banda ancha doméstica. 
Al asegurar que la validación completa puede ocurrir de manera económica, los nodos de Bitcoin
y los mineros podrán evitar la centralización extrema y la confianza, que
asegura tarifas de transacción extremadamente bajas.

Si bien es posible que la Ley de Moore continúe indefinidamente, y la
capacidad computacional para que los nodos computen de forma rentable bloques 
de varios gigabyte pueden existir en el futuro, no es una certeza.

Para lograr mucho más de 47,000 transacciones por segundo usando Bitcoin
se requiere realizar transacciones fuera del blockchain de Bitcoin. Podría ser
incluso mejor si la red bitcoin admite un número casi ilimitado de
transacciones por segundo con tarifas extremadamente bajas para micropagos. Muchos
Los micropagos se pueden enviar secuencialmente entre dos partes para permitir cualquier tamaño
de pagos. Los micropagos permitirían la desagregación, menos confianza y
mercantilización de servicios, como pagos por internet por megabyte
Servicio. Sin embargo, para poder lograr estos casos de uso de micropagos,
requieren reducir drásticamente la cantidad de transacciones que terminan siendo
transmitido en la cadena de bloques Bitcoin global.

Si bien es posible escalar a un nivel pequeño, no es posible
manejar una gran cantidad de micropagos en la red o abarcar todas las
transacciones globales. Para que bitcoin tenga éxito, se requiere confianza en que si
llegara a ser extremadamente popular, sus ventajas actuales derivadas de
la descentralización continuará existiendo. Para que las personas de hoy crean
que Bitcoin funcionará mañana, Bitcoin necesita resolver el problema del efecto de centralización
debido al tamaño de bloque; los bloques grandes crean implícitamente custodios de confianza
y tarifas significativamente más altas.

\section{Una red de canales de micropagos puede resolver la escalabilidad}

\begin{quote}
	``Si un árbol cae en el bosque y nadie está cerca para escucharlo, ¿hace un sonido?''
\end{quote}

La cita anterior cuestiona la relevancia de los eventos no observados \textemdash si
nadie oye la caída del árbol, si hizo un sonido o no no tiene ningúna
consecuencia. Del mismo modo, en el blockchain, si solo dos participantes se preocupan por
una transacción recurrente diaria, no es necesario para todos los otros nodos en
la red bitcoin saber sobre esa transacción. En cambio, es preferible
solo tener el mínimo de información en el blockchain. Aplazando
decir al mundo entero sobre cada transacción, haciendo un acuerdo neto de su
relación en una fecha posterior permite a los usuarios de Bitcoin realizar muchas transacciones
sin hinchar el blockchain o crear confianza en una contraparte 
centralizado. Se puede lograr una estructura efectivamente sin confianza
mediante el uso de bloqueos de tiempo como un componente del consenso global.

Actualmente la solución para micropagos y escalabilidad es descargar las
transacciones a un custodio, donde uno confía en terceros como custodios para
que guarden las monedas y para actualizar los saldos con otras partes. Confiar en que terceros
puedan mantener todos sus fondos crea riesgos de contraparte y costos de transacción.

En cambio, usando una red de estos canales de micropagos, Bitcoin puede escalar a
miles de millones de transacciones por día con la potencia computacional disponible en una
computadora de escritorio moderna. Envío de muchos pagos dentro de un determinado
canal de micropago le permite a uno enviar grandes cantidades de fondos a otra
parte de forma descentralizada. Estos canales no son una red de confianza separada
en la parte superior de bitcoin. Son transacciones reales de bitcoin.
